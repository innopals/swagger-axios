...
async function codegen(spec: Spec, config: CodeGenConfig) {
  if (fs.existsSync(LOCK_DIR)) {
    throw new Error(
      `Temp directory ${LOCK_DIR} exists, is there another process running?`
    );
  }
  ...
  try {
    ...
    if (config.include.length > 0) {
      apiList = apiList.filter((path) =>
        config.include.find((i) => path.startsWith(i))
      );
    }
    if (config.exclude.length > 0) {
      apiList = apiList.filter(
        (path) => !config.exclude.find((i) => path.startsWith(i))
      );
    }
    ...
    apiList.forEach((apiPath) => {
      ...
      (
        ...
        if (!op.operationId) {
          throw new Error(
            `Operation path ${apiPath} method ${method} does not have an operationId.`
          );
        }
        let axiosInstancePath =
          config.skipTags || !op.tags ? "./axiosInstance" : "../axiosInstance";
        if (config.axiosInstancePath) {
          axiosInstancePath = config.axiosInstancePath;
          if (
            axiosInstancePath.startsWith("./") ||
            axiosInstancePath.startsWith("../")
          ) {
        ...
        if (config.skipTags || !op.tags) {
          ...
          op.tags.forEach((tag) => {
            let tagFolder = tag.endsWith("-controller")
              ? tag.substr(0, tag.length - 11)
              : tag; // Springfox controller name as tag
            mkdirp.sync(path.join(LOCK_DIR, tagFolder));
            fs.writeFileSync(
              path.join(LOCK_DIR, tagFolder, op.operationId + ".ts"),
              code
            );
          });
        }
    ...
    if (config.js) {
      ...
      fs.writeFileSync(
        path.join(LOCK_DIR, "tsconfig.json"),

...
async function run() {
  try {
    ...
    console.log(
      `Using api with title: "${spec.info.title}", description: "${spec.info.description}", version: ${spec.swagger}`
    );
    if (spec.swagger !== "2.0") {
    ...
    if (!argv.force) {
      await new Promise((f) => {
        ...
        rl.question(
          `[Warning] This will wipe out the folder "${argv.out}" and re-generate client code.\nPress [ENTER] to continue or [CTRL+C] to cancel. `,
          () => {
            rl.close();
            f(0);
          }

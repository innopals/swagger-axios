...
export async function parseArgv(): Promise<CliConfig> {
  const argv = yargs
    ...
    .config("config", "Specify a config file to override defaults")
    .options({
      force: {
        alias: "f",
      ...
      out: {
        alias: "o",
        describe:
          "Specify the output directory, the whole directory will be wiped out",
        string: true,
        require: true,
      },
      include: {
        ...
        string: true,
        describe:
          "The api prefix to include, all will be included if not specified",
        default: [],
      },
      exclude: {
        alias: "x",
      ...
      "axios-instance-path": {
        describe:
          "Path to the axios instance, which handles auth, base url, result unwrapping etc. If not set, generated axios instance will be used.",
      },
    })

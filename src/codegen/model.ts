...
export function generateSchema(schema: Schema, indent: number): string {
  ...
  if (['string', 'byte', 'binary', 'date', 'dateTime', 'password'].indexOf(schema.type!) >= 0) {
    return 'string';
  }
  ...
  if (schema.type === 'array') {
    ...
    if (Array.isArray(schema.items)) {
      return `(${schema.items.map(s => generateSchema(s, indent)).join(" | ")})[]`;
    } else {
      return `${generateSchema(schema.items, indent)}[]`;
    }
  }
  if (schema.type !== 'object') {
    throw new Error(`Unexpected schema type ${schema.type}, raw: ${JSON.stringify(schema)}`);
  }
  const lines: string[] = ['{'];
